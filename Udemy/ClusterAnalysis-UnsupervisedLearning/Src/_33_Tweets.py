...
def load_the_data(csv_data_file):
...
def main():
    text, df = load_the_data("tweets/tweets.csv")
    ...
    # plt.show()
    plt.savefig("33/ward_clustering_donald_hillary")
    # fig, ax = plt.subplots()  # or
    plt.savefig('33/ward_clustering_donald_hillary_vector.eps', format='eps')
    plt.savefig("33/ward_clustering_donald_hillary_vector.svg", format='svg', dpi=300)
