...
def gmm(X, K, max_iter=20, smoothing=1e-2):
    ...
    # initializa M to random, initialize C to spherical with variance 1
    for k in range(K):
        M[k] = X[np.random.choice(N)]
        C[k] = np.eye(D)

    lls = []
    # we'll use these to store the PDF value of sample n and Gaussian k
    ...
    for i in range(max_iter):
        ...
        for k in range(K):
            ...
            M[k] = R[:, k].dot(X) / Nk
            delta = X - M[k] # N x D
            # multiplies R[:, k] by each col of delta - X x D
    ...
    plt.savefig("35/Log-Likelihood")

    plt.clf()
    random_colors = np.random.random((K, 3))
    colors = R.dot(random_colors)
    plt.scatter(X[:,0], X[:,1], c=colors)
    plt.savefig("35/scatter")
